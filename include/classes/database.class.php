...
class database{
    ...
    /**
      * variable yang baru ditambahkan
      * script dimulai
    */
	var $db_fetch_object="";
	var $db_num_fields="";
	...
	*/
	function database($database_type,$host,$database,$user,$password,$port=false,$dsn=false){
		...
		if(in_array($database_type,$this->database_types)){
             ...
             * script dimulai
            */
			$this->db_num_fields=$this->database_type."_num_fields";
			$this->db_fetch_field=$this->database_type."_fetch_field";
			$this->db_fetch_object=$this->database_type."_fetch_object";
			/**
             * script berakhir
             */
	...
	*/
	...
	/**
	* This function returns fields in the resultset
	* @return array $namerow the array of field name in the resultset
	* @desc This function returns array of fied name in the resultset
	*/
	function get_field(){
		$i = 0;
		while ($i < $this->count_fields()) {
		$row=call_user_func($this->db_fetch_field,$this->query_id, $i);
		$namerow[$i]= $row->name;
		$i++;
		}
		return $namerow;
    ...
    /**
	* This function returns number of field in the resultset
	* @return int $row_field the number of field in the resultset
	* @desc This function returns number of field in the resultset
	*/
    
   	function count_fields(){
		$row_field=call_user_func($this->db_num_fields,$this->query_id);
		if($row_field>=0){
			return $row_field;
		}else{
			$this->halt("Can't count rows before query was made");
	...
	* @return int $array the objest of row in the resultset
	* @desc This function returns object of row in the resultset
	*/
    
    function get_object(){
		$array = array();
		while ($row = call_user_func($this->db_fetch_object,$this->query_id)) {
			if ($key) {
				$array[$row[$key]] = $row;
			} else {
				$array[] = $row;
